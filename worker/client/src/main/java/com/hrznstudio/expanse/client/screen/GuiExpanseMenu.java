...
public class GuiExpanseMenu extends Screen {
    ...
    @Nullable
    private GuiExpanseMenu.Warning warning;
    ...
    public GuiExpanseMenu() {
        this(false);
    }

    public GuiExpanseMenu(boolean boolean_1) {
...
//            this.warning = new GuiExpanseMenu.Warning((new TranslatableText("title.oldgl.eol.line1")).formatted(Formatting.RED).formatted(Formatting.BOLD), (new TranslatableText("title.oldgl.eol.line2")).formatted(Formatting.RED).formatted(Formatting.BOLD), "https://help.mojang.com/customer/portal/articles/325948?ref=game");
    ...
    private void initWidgetsDemo(int int_1, int int_2) {
        ...
        this.buttonResetDemo = this.addButton(new ButtonWidget(this.width / 2 - 100, int_1 + int_2 * 1, 200, 20, I18n.translate("menu.resetdemo"), (buttonWidget_1) -> {
    ...
    class Warning {
        ...
        public Warning(Text text_1, Text text_2, String string_1) {
            this.line1 = text_1;
            this.line2 = text_2;
            this.helpUrl = string_1;
        }

        public void init(int int_1) {
            int int_2 = GuiExpanseMenu.this.font.getStringWidth(this.line1.getString());
            this.line2Width = GuiExpanseMenu.this.font.getStringWidth(this.line2.getString());
            int int_3 = Math.max(int_2, this.line2Width);
            this.startX = (GuiExpanseMenu.this.width - int_3) / 2;
        ...
        public boolean onClick(double double_1, double double_2) {
            if (!ChatUtil.isEmpty(this.helpUrl) && double_1 >= (double)this.startX && double_1 <= (double)this.endX && double_2 >= (double)this.startY && double_2 <= (double)this.endY) {
                GuiExpanseMenu.this.minecraft.openScreen(new ConfirmChatLinkScreen((boolean_1) -> {
                    if (boolean_1) {
                        SystemUtil.getOperatingSystem().open(this.helpUrl);
                    }

                    GuiExpanseMenu.this.minecraft.openScreen(GuiExpanseMenu.this);
                }, this.helpUrl, true));
                return true;
            } else {
                return false;
